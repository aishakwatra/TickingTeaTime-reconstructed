...
TEST_CASE("TotalScore - good food on time stays INFORMATIVE", "[CharacterData][normal]")
{
    // mealScore=10, on time → total=10 → INFORMATIVE
    REQUIRE(ClassifyScore(TotalScore(10, 30)) == INFORMATIVE);
}

TEST_CASE("TotalScore - average food on time stays AVERAGE", "[CharacterData][normal]")
{
    // mealScore=6, on time → total=6 → AVERAGE
    REQUIRE(ClassifyScore(TotalScore(6, 30)) == AVERAGE);
}

TEST_CASE("TotalScore - low food on time stays LOW", "[CharacterData][normal]")
{
...
TEST_CASE("ScoreLevel - boundary: score 8 gives INFORMATIVE", "[CharacterData][edge]")
{
    REQUIRE(ClassifyScore(8) == INFORMATIVE);
}

TEST_CASE("TotalScore - late penalty pushes boundary score down one level", "[CharacterData][edge]")
{
    // mealScore=4 (lowest AVERAGE) - 2 penalty = 2 → LOW
    REQUIRE(ClassifyScore(TotalScore(4, 0)) == LOW);
}

TEST_CASE("TotalScore - late penalty pushes boundary INFORMATIVE to AVERAGE", "[CharacterData][edge]")
{
    // mealScore=8 (lowest INFORMATIVE) - 2 = 6 → AVERAGE
    REQUIRE(ClassifyScore(TotalScore(8, 0)) == AVERAGE);
}

TEST_CASE("TotalScore - late penalty on already LOW stays LOW", "[CharacterData][edge]")
{
...
TEST_CASE("TotalScore - worst food late gives lowest possible score", "[CharacterData][failure]")
{
...
TEST_CASE("TotalScore - good food but late can lose INFORMATIVE", "[CharacterData][failure]")
{
